...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="1.0">
  <xsl:variable name="filename">XSD2PDF_X050_de-DE_4.4.1_6.0.1.xslt</xsl:variable>
        ...
        <fo:static-content flow-name="xsl-region-before">
          <fo:table table-layout="fixed" width="100%" margin="5mm 0mm 0mm 0mm">
            ...
            <fo:table-body>
              <fo:table-row>
                <fo:table-cell>
                  <fo:block font-family="Arial Unicode MS" margin-left="5mm" font-size="14pt" text-align="start">X050 - Ausnahmesituation im Verfahrensablauf</fo:block>
                  <fo:block font-family="Arial Unicode MS" margin-left="5mm" font-size="8pt" text-align="start">X050 - Business Exception</fo:block>
                </fo:table-cell>
                <fo:table-cell>
          ...
          <fo:block>
            <fo:table table-layout="fixed" width="100%" margin="2mm 0mm 2mm 2mm" border="solid 0.2mm Grey">
              ...
              <fo:table-header background-color="LightGrey">
                <fo:table-row border="solid 0.2mm Grey">
                  <fo:table-cell number-columns-spanned="2" padding="1mm 0mm 1mm 0mm" font-weight="bold">
                    <fo:block>2. Folgende Ausnahmesituation im Verfahrensablauf ist eingetreten</fo:block>
                  </fo:table-cell>
                </fo:table-row>
              </fo:table-header>
              <fo:table-body start-indent="2mm">
                <fo:table-row>
                  <fo:table-cell border-top="solid 0.2mm Grey" border-bottom="solid 0.2mm Grey" padding="1mm 0mm 1mm 0mm">
                    <fo:block wrap-option="wrap">2.1. Ausnahmesituation im Verfahrensablauf</fo:block>
                  </fo:table-cell>
                  <fo:table-cell border-top="solid 0.2mm Grey" border-bottom="solid 0.2mm Grey" padding="1mm 0mm 1mm 0mm">
                    <fo:block>
                      <xsl:value-of select="//TheFollowingBusinessExceptionHasOccurred/businessException"/>
                      <fo:block>
                        <xsl:for-each select="//TheFollowingBusinessExceptionHasOccurred/businessException/value">
                          <xsl:choose>
                            <xsl:when test=".='01'">
                              <fo:block>Erhalt eines SED für einen fehlenden Fall</fo:block>
                            </xsl:when>
                            <xsl:when test=".='02'">
                              <fo:block>Erhalt einer Aktualisierung zu einem fehlenden SED</fo:block>
                            </xsl:when>
                            <xsl:when test=".='03'">
                              <fo:block>Erhalt eines SED, nachdem der Fall an einen anderen Teilnehmer weitergeleitet wurde</fo:block>
                            </xsl:when>
                            <xsl:when test=".='04'">
                              <fo:block>Erhalt eines SED nachdem der Fall durch den Erhalt eines SED X006 gelöscht wurde</fo:block>
                            </xsl:when>
                            <xsl:when test=".='05'">
                              <fo:block>SED wurde nicht in der richtigen Reihenfolge gemäß BUC erhalten.</fo:block>
                            </xsl:when>
                            <xsl:when test=".='06'">
                              <fo:block>Erhalt eines SED bei einem Fall mit global geschlossenem Status</fo:block>
                            </xsl:when>
                            <xsl:when test=".='07'">
                              <fo:block>Erhalt eines SED mit einer ungültigen Business Signatur</fo:block>
                            </xsl:when>
                            <xsl:when test=".='08'">
                              <fo:block>Erhalt eines SED/eines Anhangs, das/der die Anti-Malware-Prüfung nicht bestanden hat</fo:block>
                            </xsl:when>
                            <xsl:otherwise>
                              <fo:block>-/-</fo:block>
                            </xsl:otherwise>
          ...
          <fo:block>
            <fo:table table-layout="fixed" width="100%" margin="2mm 0mm 2mm 2mm" border="solid 0.2mm Grey">
              ...
              <fo:table-header background-color="LightGrey">
                <fo:table-row border="solid 0.2mm Grey">
                  <fo:table-cell number-columns-spanned="2" padding="1mm 0mm 1mm 0mm" font-weight="bold">
                    <fo:block>3. Bei Erhalt des folgenden SED:</fo:block>
                  </fo:table-cell>
                </fo:table-row>
              </fo:table-header>
              <fo:table-body start-indent="2mm">
